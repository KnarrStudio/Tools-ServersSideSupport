function Copy-PuTTYSettings #Incomplete - inop
{
  ...
  Write-Host -Object 'This will allow you to copy all of the settings from one session to all of the other sessions. ' -ForegroundColor Cyan
  Return
...
'@
    ...
    Start-Process notepad $InputFileName
    return
  }

  
  $RegistryFile = (New-RegistryFile -FileName $OutputFileName  -outputPath $OutputFolder).FullName
...
function Import-PuTTYSessions #Completed
{
  ...
  try
  {
    if(-not $CreateTemplate)
    {
      $ImportedData = Import-Csv -Path $File -ErrorAction Stop
    }
  }
...
function Export-PuTTYSessions #Completed
{
  ...
  Get-ItemProperty -Path ('HKCU:{0}' -f $($item.TrimStart('HKEY_CURRENT_USER'))) | Out-File -FilePath $outputfile  -Append3
      Export-SessionToFile -outputfile $outputfile -item $item
    }
  }
  else
  {
    #$outputfile = New-RegistryFile -FileName 'PuttyBundle'
    foreach($item in $PuTTYSessions)
    {
      $itemName = Split-Path -Path $item -Leaf
      Export-SessionToFile -outputfile $outputfile -item $item
...
function New-RegistryFile
{
  ...
  {
    $outputfile = $FileName.Replace($FileExt,('({0}).reg' -f $i))
  }
